...
def encoder(
    ...
    for h in range(num_heads):
        ...
        is_first_head = (h == 0)
        addr_add = input_addr if is_first_head else attention_buf
    ...
    instr_id = LayerNorm(
        input_addr=attention_buf, output_addr=attention_buf,
        ...
        instr_id=0,
        layer_name="LayerNorm0"
    )
    instrs.append(Instruction.End())

    instr_id = matmul_gelu(
        addr_a_base=attention_buf, addr_b_base=Wi_addr, addr_c_base=GeLU_buf,
        Width_A=hidden_dim, Height_A=seq_len,
    ...
    instr_id = matmul_add(
        addr_a_base=GeLU_buf, addr_b_base=Weo_addr, addr_c_base=context_buf, addr_add_base=attention_buf,
    ...
    instr_id = LayerNorm(
        input_addr=context_buf, output_addr=output_addr,
        input_height=seq_len, input_width=hidden_dim,
