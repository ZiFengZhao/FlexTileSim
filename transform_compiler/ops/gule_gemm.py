...
def matmul_sa_tile(addr_a, addr_b, addr_c,
    ...
    gelu_lbuf= lbuf_base + 6 * lbuf_stride 
    ...
    instrs.append(Instruction.RET(
        ...
        shift = shift,
        scale = scale,
        dep = ("01" if has_bias else "00"),
        instr_id = instr_id,
        comment = f"[{layer_name}] RET C(i={i}, j={j})"
    ))
    instr_id += 1
    # GeLU prod(10)
    instrs.append(Instruction.Gelu(
        ocm_addr_out = gelu_lbuf,
        ocm_addr_in = c_lbuf,
        de_shift = gelu_re_shift, de_scale = gelu_re_scale,
        shift = gelu_shift, scale = gelu_scale,
        dep = "10",
        instr_id = instr_id,
        comment = f"[{layer_name}] GeLU C(i={i}, j={j})"
    ))
    instr_id += 1
    # Mvout C cons(01)
