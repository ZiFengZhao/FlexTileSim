...
def matmul_relu(addr_a_base, addr_b_base, addr_c_base,
...
def matmul_sa_tile(addr_a, addr_b, addr_c,
    ...
    relu_lbuf= lbuf_base + 6 * lbuf_stride  
    ...
    instr_id += 1
    instrs.append(Instruction.Relu(
        ocm_addr_out = relu_lbuf,
        ocm_addr_in = c_lbuf,
        dep = "10",
        instr_id = instr_id,
        comment = f"[{layer_name}] ReLU C(i={i}, j={j})"
    ))
    instr_id += 1
