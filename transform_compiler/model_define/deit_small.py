...
from ops.pre_encoder import pre_encoder
...
def main():
    ...
    SZ_PROJ = hidden_dim * hidden_dim
    SZ_FFN  = hidden_dim * (hidden_dim * 4)
    SZ_VEC  = hidden_dim
    
    LAYER_WEIGHT_STRIDE = 0x400000  
    LAYER_BIAS_STRIDE   = 0x100000  
    
    ping_addr = DRAM_BASE_C
    pong_addr = DRAM_BASE_C + 0x200000 
    
    internal_buf_base = DRAM_BASE_C + 0x400000 
    
    Q_buf         = internal_buf_base
    K_buf         = Q_buf + 0x40000       
    V_buf         = K_buf + 0x40000
    score_buf     = V_buf + 0x40000        
    context_buf   = score_buf + 0x100000
    attention_buf = context_buf + 0x40000
    GeLU_buf      = attention_buf + 0x100000 
    ...
    for i in range(num_layers):
        ...
        Beo_addr    = Bi_addr + (hidden_dim * 4)
        beta1_addr  = Beo_addr + SZ_VEC
        
        layer_instrs = pre_encoder(
            ...
            Wq_addr=Wq_addr, Wk_addr=Wk_addr, Wv_addr=Wv_addr, Wo_addr=Wo_addr,
            gamma_addr=gamma_addr, Wi_addr=Wi_addr, Weo_addr=Weo_addr, gamma1_addr=gamma1_addr,
            
            Bq_addr=Bq_addr, Bk_addr=Bk_addr, Bv_addr=Bv_addr, Bo_addr=Bo_addr,
            beta_addr=beta_addr, Bi_addr=Bi_addr, Beo_addr=Beo_addr, beta1_addr=beta1_addr,
    ...
    print(f"// DeiT-Small (Hardware Compatible)")
    print(f"// Config: {num_layers} layers, {hidden_dim}d, {num_heads} heads, {seq_len} tokens")
    print(f"// Hardware constraints: hidden_dim % 128 == 0, seq_len % 128 == 0")
    print(f"// Note: Original seq_len=197 padded to {seq_len}")
    print(f"// Final output addr: 0x{final_output:X}")
