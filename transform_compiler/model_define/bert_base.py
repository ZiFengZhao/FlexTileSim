...
def main():
    ...
    SZ_FFN  = 0x240000
    SZ_VEC  = 0x1000
    LAYER_WEIGHT_STRIDE = 0x800000
    LAYER_BIAS_STRIDE   = 0x100000

    ping_addr = DRAM_BASE_C
    pong_addr = DRAM_BASE_C + 0x100000
    internal_buf_base = DRAM_BASE_C + 0x200000

    Q_buf         = internal_buf_base
    K_buf         = internal_buf_base + 0x20000
    V_buf         = internal_buf_base + 0x40000
    score_buf     = internal_buf_base + 0x60000
    context_buf   = internal_buf_base + 0x80000
    attention_buf = internal_buf_base + 0xA0000
    GeLU_buf      = internal_buf_base + 0xC0000
    ...
    print(f"// BERT-Base instructions generated. Final output addr: 0x{final_output:X}")
    for inst in instrs:
        print(inst)
